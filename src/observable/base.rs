...
pub trait LocalPublisherFactory<'a>: PublisherFactory {
  fn subscribe<O>(self, subscriber: Subscriber<O, LocalSubscription<'a>>) -> LocalSubscription<'a>
    where
        O: Observer<Item = Self::Item, Err = Self::Err> + 'a;
}

pub trait SharedPublisherFactory: PublisherFactory {
  fn subscribe<O>(self, subscriber: Subscriber<O, SharedSubscription>) -> SharedSubscription
    where
