...
pub struct IterPublisher<Iter, Item: Send + 'static>
  where
      Iter: IntoIterator<Item=Item>,
{
  it: Iter,
  cursor: usize,
  observer: Sender<SubscriberChannelItem<Iter::Item, ()>>,
}
    ...
    Iter: IntoIterator<Item=Item> + Send + Sync + Clone + 'static,
{
  fn subscribe<S>(self, mut subscriber: S) where
    ...
    let mut publisher = IterPublisher {
      it: self.0,
      cursor: 0,
      observer: *ch.item_chn,
    };
    start_publish_loop2(publisher, *ch.request_chn);
